import { fromEvent, map, merge, publishReplay, refCount, ReplaySubject } from "rxjs";
...
export class ObservableSocket {
	...
	constructor(socket) {
		...
		this.status$ = merge(
			this.on$("connect").pipe(map(() => ({ isConnected: true }))),
			this.on$("disconnect").pipe(map(() => ({ isConnected: false }))),
			this.on$("reconnecting").pipe(map(attempt => ({ isConnected: false, isReconnecting: true, attempt }))),
			this.on$("reconnect_failed").pipe(map(() => ({ isConnected: false, isReconnecting: false }))))
	...
	_registerCallbacks(action) {
		if (Object.prototype.hasOwnProperty.call(this._actionCallbacks, action))
			return;
	...
	_popRequest(id) {
		if (!Object.prototype.hasOwnProperty.call(this._requests, id)) {
